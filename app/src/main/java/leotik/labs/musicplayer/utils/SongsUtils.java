package leotik.labs.musicplayer.utils;
...
public class SongsUtils {
    ...
    public List<SongModel> searchYoutube(String query){

        List<SongModel> songs = new ArrayList<>();
        try {
            YoutubeSearchExtractor secondExtractor = (YoutubeSearchExtractor) YouTube.getSearchExtractor(query,
                    asList(YoutubeSearchQueryHandlerFactory.VIDEOS), null);
            secondExtractor.fetchPage();
            ListExtractor.InfoItemsPage<InfoItem> page = secondExtractor.getInitialPage();
    ...
    public void play(int id, ArrayList<SongModel> array){
        ...
        if (!array.isEmpty()) {
            ...
            else if(array.get(id).getPath().startsWith("http")){
                replaceQueue(array);
                setCurrentMusicID(id);
                Intent intent = new Intent(MusicPlayback.ACTION_PLAY);
                ContextCompat.startForegroundService(context, createExplicitFromImplicitIntent(intent));
